...
- FastAPI app exposing a thin wrapper over the OpenAI Responses API and the `openai-agents` SDK.
- No test suite, no linter, no formatter, no CI config in this repo — don't invent commands.
...
- `app/routers/` — one file per endpoint group (`chat.py`, `agent.py`, `stream.py`). Each handler is a near-pure adapter: parse `ChatRequest`, call a service, wrap the result in `ChatResponse` (or an `EventSourceResponse` for SSE).
- `app/services/` — the actual OpenAI / agents SDK calls. Each service defines its own `MODEL = "gpt-4o-mini"` constant — keep them in sync if you change models.
...
- `app/schemas/chat.py` — Pydantic models `ChatRequest` / `ChatResponse`. New request/response shapes go here.
...
- The frontend pulls types straight from the dumped `openapi.json`. If you add or change a schema, regenerate by restarting the server — don't hand-edit `openapi.json` (it's regenerated on every startup and is gitignored in spirit, though currently tracked).
